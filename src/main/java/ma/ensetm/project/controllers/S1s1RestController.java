...
public class S1s1RestController {
    @Autowired
    ...
    @PostMapping("/import-s1s1")
    public Map<String, Object> uploadFileS1s1(@RequestBody Map<String, Object> toBeImported) {
    ...
    @PostMapping("/export-s1s1")
    public Map<String, Object> exportForS1s1(@RequestBody Data output, @RequestParam(required = false, defaultValue = "true") String avecElement) {
        ...
        try {
            theSavedFile = new FileInputStream(new File(new File("").getAbsolutePath() + "/templates/TemplateS1S1.xls"));
