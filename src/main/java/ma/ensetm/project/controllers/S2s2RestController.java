...
public class S2s2RestController {
    @Autowired
    ...
    @PostMapping("/import-s2s2")
    public Map<String, Object> uploadFileS2s2(@RequestBody Map<String, Object> toBeImported) {
        ...
        try {
            ...
            try {
                ...
                for (ArrayList<String> studentLine : studentLines) {
                    ...
                    markService.save(new Mark(null,
                            element15.getNote(), "S2", elementService.findByCode(element15.getCode()),
                            studentService.findById(Long.parseLong(studentLine.get(0)))));
    ...
    @PostMapping("/export-s2s2")
    public Map<String, Object> exportForS2s2(@RequestBody Data output, @RequestParam(required = false, defaultValue = "true") String avecElement) {
        ...
        try {
            theSavedFile = new FileInputStream(new File(new File("").getAbsolutePath() + "/templates/TemplateS2S2.xls"));
        ...
        try {
            DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy-HH-mm-ss");
            String name = "S2S2-" + dateFormat.format(new Date()) + ".xls";
